...
export function AiSummaryPanel() {
  const { analytics, aiInsight, setAiInsight, setError } = useAnalyticsStore();
  ...
  return (
    <section className="metric-panel p-4">
      <div className="mb-4 flex items-center justify-between">
        ...
        <span className="text-xs text-[#5b635f]">Aggregates only</span>
      </div>

      <div className="grid gap-3">
        <div className="rounded border border-[#d9ded8] bg-[#f7f7f2] p-3 text-sm text-[#4f5954]">
          {token
            ? "AI is configured for this app. Only aggregate analytics are sent for explanation."
            : "AI is not configured for this build. Set NEXT_PUBLIC_HF_TOKEN for the app owner build."}
        </div>
      ...
      {aiInsight ? (
        <div className="mt-5 flex flex-col gap-4 border-t border-[#d9ded8] pt-4 text-sm">
          ...
          <InsightList title="Trends" items={aiInsight.trends} />
          <InsightList title="Instructional Focus" items={aiInsight.instructionalFocus} />
          <InsightList title="Cautions" items={aiInsight.cautions} />
          ...
          .map((suggestion, index) => (
            <div key={`${suggestion.title}-${index}`} className="rounded border border-[#d9ded8] bg-[#f7f7f2] p-3">
              <div className="flex items-center justify-between gap-2">
                <span className="font-semibold">{toPanelText(suggestion.title)}</span>
                <span className="rounded bg-[#e4edf5] px-2 py-1 text-xs font-medium text-[#254f78]">
                  {toPanelText(suggestion.type)}
                </span>
              </div>
